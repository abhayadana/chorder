-- lib/patternlib.lua
-- Pattern library for CHORDER — extracted module
-- Public API:
--   GENRES : string[]
--   NAMES_FOR(genre) : string[]
--   GET(genre, idx) : pattern | nil
--   RAND(genre) : pattern | nil
--
-- Pattern schema (two kinds):
--   1) Basics adapter (defers to strum order)
--        { name=..., basics=true, use_order=true }
--        or { name=..., basics=true, legacy="up" | "down" | ... }
--   2) Degree/overtone patterns
--        { name=..., steps={ {d=1..7, oct=0, prob=1.0, r=1, rest=false}, ... } }

local M = {}

-- convenience constructor for a pattern step
...
-- small helpers to seed common shapes
...
-- expand seeds into a richer library (variants, rests, prob, ratchets)
...
local function patterns_for(genre)
  local t = {}
  if LIB[genre] then
    for _,p in ipairs(LIB[genre]) do t[#t+1]=p.name end
  end
